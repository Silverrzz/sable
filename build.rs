...
fn workspace_default_weights(out_dir: &Path) -> PathBuf {
    ...
    let source = out_dir.join("sable-dev-net-6");
    if source.is_file() {
        validate_downloaded_weights(&source);
        return source;
    }

    let url = "https://raw.githubusercontent.com/Silverrzz/sable-nets/main/sable-dev-net-6";
    let temporary = out_dir.join("sable-dev-net-6.part");
...
fn validate_downloaded_weights(path: &Path) {
    ...
    let tensor_bytes = (6144 + 1) * 1024 * size_of::<i16>() + (2048 + 1) * size_of::<f32>();
    assert!(
        (tensor_bytes..=tensor_bytes + 63).contains(&bytes.len())
            && bytes[tensor_bytes..].iter().enumerate()
                .all(|(index, &byte)| byte == b"bullet"[index % 6]),
        "Downloaded network '{}' does not match the expected 6144x1024 float32-score Bullet format",
        path.display()
    );
}
...
fn display_label(source: &Path) -> String {
    ...
    if source.file_name().is_some_and(|name| name == "sable-dev-net-6") {
        return "sable-dev-net-6".to_owned();
    }
