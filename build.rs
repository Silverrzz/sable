...
fn workspace_default_weights(out_dir: &Path) -> PathBuf {
    ...
    let source = out_dir.join("sable-dev-net-10.bin");
    if source.is_file() {
        validate_downloaded_weights(&source);
        return source;
    }

    let url = "https://raw.githubusercontent.com/Silverrzz/sable-nets/main/nets/sable-dev-net-10.bin";
    let temporary = out_dir.join("sable-dev-net-10.bin.part");
...
fn display_label(source: &Path) -> String {
    ...
    if source.file_name().is_some_and(|name| name == "sable-dev-net-10.bin") {
        return "sable-dev-net-10.bin".to_owned();
    }
